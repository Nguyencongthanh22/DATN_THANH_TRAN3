...
class paymentScreenCard extends StatefulWidget {
  ...
  @override
  State<paymentScreenCard> createState() => _PaymentScreenState();
}
...
class _PaymentScreenState extends State<paymentScreenCard> {
  @override
  void initState() {
    super.initState();
  ...
  double calculateTotal() {
    ...
    for (var product in Cardproducts) {
      ...
      id_sp = product.id_sp;
      gia = double.parse(product.gia ?? '0');
  ...
  Future<void> AddOrders2() async {
    final data = {
      'email': email,
      'tongtien': tong = total - phivanchuyen,
      'diachi': diachi,
  ...
  Future<void> addOrderDetail() async {
    for (var product in Cardproducts) {
      ...
      try {
        ...
        if (response['status'] == 200) {
          print('Order detail added successfully for product ${product.tensp}');
        } else {
